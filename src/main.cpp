...
int main() {
    ...
    // ustawienie auta gracza na starcie (CircuitMode zajmie sie ustawieniem pozycji na mapie)
    ...
    // respawn botow z uzyciem inteligentnych wskaznikow pod CircuitMode
    ...
    for (int i = 0; i < 4; i++) {
        ...
        // przypisanie trasy i pozycji startowej wykona teraz metoda CircuitMode::init
    }

    // inicjalizacja trybu wyscigu
    CircuitMode circuit;
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::KeyPressed) {
                ...
                if (!inGarage) {
                    // reset wyscigu i odtworzenie aut na starcie calego toru
                    if (event.key.code == sf::Keyboard::R) {
                        circuit.init(&myCar, bots);
                        raceFinishedHandled = false;
                    } else {
                        // przekazanie reszty wejscia (biegi, nitro) do trybu wyscigu
        ...
        if (inGarage) {
            ...
            // cala logika fizyki, botow, collectibli, HUD i kamery zaktualizowana przez tryb
            circuit.update(dt);
        ...
        if (inGarage) {
            ...
            // cale rysowanie swiata, pojazdow, efektow czasteczkowych i HUD delegowane do klasy
            circuit.draw(window);
